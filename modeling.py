...
from utils.utils import extract_clusters, extract_mentions_to_predicted_clusters_from_clusters, mask_tensor
...
class S2E(BertPreTrainedModel):
    ...
    def forward(self, input_ids, attention_mask=None, gold_clusters=None, return_all_outputs=False): # input_ids: [batch_size, seq_len]
        outputs = self.longformer(input_ids, attention_mask=attention_mask) # ([batch_size, seq_len, dim], [batch_size, dim])
        sequence_output = outputs[0]  # [batch_size, seq_len, dim]

        # Compute representations
        start_mention_reps = self.start_mention_mlp(sequence_output) if self.do_mlps else sequence_output # [batch_size, seq_len,  3dim]
        end_mention_reps = self.end_mention_mlp(sequence_output) if self.do_mlps else sequence_output  # [batch_size, seq_len,  3dim]

        start_coref_reps = self.start_coref_mlp(sequence_output) if self.do_mlps else sequence_output # [batch_size, seq_len,  3dim]
        end_coref_reps = self.end_coref_mlp(sequence_output) if self.do_mlps else sequence_output # [batch_size, seq_len,  3dim]
        ...
        final_logits = self._mask_antecedent_logits(final_logits, span_mask) # [BS, maxk, maxk]
